# imported from https://github.com/lucidrains/g-mlp-pytorch/blob/main/g_mlp_pytorch/g_mlp_pytorch.py
...
class gMLP(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.to_embed = nn.Embedding(num_tokens_in, dim) if exists(num_tokens_in) else nn.Identity()
        ...
        self.vq_embed_proj = nn.Linear(vq_embedding_dim, dim) if vq_embedding_dim is not None else None
        self.to_logits = nn.Sequential(
            nn.LayerNorm(dim),
            nn.Linear(dim, num_tokens_out)
        ) if exists(num_tokens_out) else nn.Identity()

    def forward(self, x, vq_embeddings):
        if self.vq_embed_proj is not None:
            x = self.to_embed(x) + self.vq_embed_proj(vq_embeddings)
        else:
            x = self.to_embed(x)
