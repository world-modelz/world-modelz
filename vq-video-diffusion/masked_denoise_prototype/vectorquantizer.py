...
class VectorQuantizerEMA(nn.Module):
    def __init__(self, embedding_dim, num_embedding, num_latents, decay=0.99, eps=1e-5):
        super().__init__()
    ...
    # hack
    def add_zero_mask_token(self):
        new_embedding = torch.zeros(self.num_latents, self.num_embeddings+1, self.embedding_dim, device=self.embedding.device)
        new_embedding[:self.num_latents, :self.num_embeddings, :self.embedding_dim] = self.embedding
    ...
    def codebook_distance(self, input, normalize=True):
        flat_input = input.view(-1, self.num_latents, self.embedding_dim)
        distances = (flat_input.unsqueeze(-1) - self.embedding[:,:self.num_embeddings].transpose(1, 2).unsqueeze(0)).pow(2).sum(dim=-2)
        if normalize:
            distances = distances / self.embedding_dim[:,:self.num_embeddings]
        return distances
    ...
    def decode(self, indices):
        ...
        num_embeddings = self.embedding.size(1)
        quantized = self.embedding.view(self.num_latents * num_embeddings, self.embedding_dim)[flat_index]
    ...
    def reuse_inactive(self):
        ...
        for i in range(self.num_latents):
            ...
            num_dead = dead_mask.count_nonzero()
            if num_dead > 0:
